  ...
  change do
    create_table(:goals) do
      ...
      constraint(:goals_repeat_finish_presence) do
        Sequel.case(repeat: Sequel.lit('finish IS NOT NULL'), true)
      end
    ...
    comment_on :column, :goals__parent_id, 'This is both to explicit the repeat parent/child relationships, and to disambiguate whether a goal should be considered past (“has a child”) in some odd timezone-of-database-server-related cases'
    ...
    comment_on :column, :goals__start, 'A date, re-interpreted in application code to mean the right thing in the user’s current timezone'
